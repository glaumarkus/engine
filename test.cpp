...
TEST(Test, Aes256GCM) {
  ...
  ret = EVP_EncryptUpdate(ctx, ciphertext_en.data(), &ciphertext_len,
                          reinterpret_cast<const uint8_t *>(plaintext.data()),
                          plaintext.size());
  EXPECT_TRUE(ret);
...
//   "/home/glaum/engine/keys/alice_pubkey.pem"; std::size_t engine_secret_len =
//   0;
...
//   ret = EVP_PKEY_derive(ctx1, shared_secret_alice.data(),
//   &engine_secret_len); shared_secret_alice.resize(engine_secret_len);
//   EXPECT_EQ(ret, 1);
...
//   EXPECT_EQ(ret, 1);
//   ret = EVP_DigestVerifyUpdate(mdctx, (unsigned char *)msg.data(),
//   msg.size()); EXPECT_EQ(ret, 1); ret = EVP_DigestVerifyFinal(mdctx,
//   (unsigned char *)signature.data(), siglen); EXPECT_EQ(ret, 1);
//   EVP_MD_CTX_free(mdctx);
...
//   // sign with engine
//   mdctx = EVP_MD_CTX_new();
//   ret = EVP_DigestSignInit(mdctx, nullptr, EVP_sha256(), engine,
//   pkey_engine); EXPECT_EQ(ret, 1); ret = EVP_DigestSignUpdate(mdctx,
//   (unsigned char *)msg.data(), msg.size()); EXPECT_EQ(ret, 1);
...
//   ret = EVP_DigestVerifyUpdate(mdctx, (unsigned char *)msg.data(),
//   msg.size()); EXPECT_EQ(ret, 1); ret = EVP_DigestVerifyFinal(mdctx,
//   (unsigned char *)signature_engine.data(),
//                               siglen);
//   EXPECT_EQ(ret, 1);
//   EVP_MD_CTX_free(mdctx);
...
//   mdctx = EVP_MD_CTX_new();
//   ret = EVP_DigestVerifyInit(mdctx, nullptr, EVP_sha256(), engine,
//   pkey_engine); EXPECT_EQ(ret, 1); ret = EVP_DigestVerifyUpdate(mdctx,
//   (unsigned char *)msg.data(), msg.size()); EXPECT_EQ(ret, 1); ret =
//   EVP_DigestVerifyFinal(mdctx, (unsigned char *)signature_engine.data(),
//                               siglen);
//   EXPECT_EQ(ret, 1);
//   EVP_MD_CTX_free(mdctx);
...
//   pkey_engine); EXPECT_EQ(ret, 1); ret = EVP_DigestVerifyUpdate(mdctx,
//   (unsigned char *)msg.data(), msg.size()); EXPECT_EQ(ret, 1); ret =
//   EVP_DigestVerifyFinal(mdctx, (unsigned char *)signature.data(),
//                               signature.size());
//   EXPECT_EQ(ret, 1);
//   EVP_MD_CTX_free(mdctx);
...
//   pubkey_engine =
//       ENGINE_load_public_key(engine, path_to_pubkey.c_str(), nullptr,
//       nullptr);
//   EXPECT_NE(pkey_engine, nullptr);
...
//   mdctx = EVP_MD_CTX_new();
//   ret =
//       EVP_DigestVerifyInit(mdctx, nullptr, EVP_sha256(), engine,
//       pubkey_engine);
//   EXPECT_EQ(ret, 1);
//   ret = EVP_DigestVerifyUpdate(mdctx, (unsigned char *)msg.data(),
//   msg.size()); EXPECT_EQ(ret, 1); ret = EVP_DigestVerifyFinal(mdctx,
...
//   EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
//   EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr,
//                      (unsigned char *)key.data(), (unsigned char
//                      *)iv.data());
...
//   // Encryption with Openssl
//   EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
//   EVP_EncryptInit_ex(ctx, EVP_chacha20(), nullptr, (unsigned char
//   *)key.data(),
...
//   decrypted_sw.resize(ciphertext_len);
//   ctx = EVP_CIPHER_CTX_new();
//   EVP_DecryptInit_ex(ctx, EVP_chacha20(), nullptr, (unsigned char
//   *)key.data(),
//                      (unsigned char *)nonce.data());
//   int plaintext_len = 0;
//   EVP_DecryptUpdate(ctx, (unsigned char *)decrypted_sw.data(),
...
//   // encrypt with engine
//   ctx = EVP_CIPHER_CTX_new();
//   EVP_EncryptInit_ex(ctx, EVP_chacha20(), engine, (unsigned char
//   *)key.data(),
//                      (unsigned char *)nonce.data());
...
//   decrypted_engine.resize(ciphertext_len);
//   ctx = EVP_CIPHER_CTX_new();
//   EVP_DecryptInit_ex(ctx, EVP_chacha20(), engine, (unsigned char
//   *)key.data(),
//                      (unsigned char *)nonce.data());
//   int plaintext_len_engine = 0;
//   EVP_DecryptUpdate(
