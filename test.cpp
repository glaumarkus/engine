...
//   int engine_load = ENGINE_ctrl_cmd(engine, cmd_name, 0, &params, NULL, 1);
//   EXPECT_EQ(1, engine_load);

//   // check if equal
//   int equal = X509_cmp(cert_sw, params.cert);
//   EXPECT_EQ(1, equal);
// }
...
//     EVP_DigestUpdate(evp_ctx, (unsigned char*)str.data(), str.size());
//     EVP_DigestFinal(evp_ctx, (unsigned char*)digest_engine.data(),
//     &digestSize);
...
//     EVP_DigestUpdate(evp_ctx, (unsigned char*)str.data(), str.size());
//     EVP_DigestFinal(evp_ctx, (unsigned char*)digest_engine.data(),
//     &digestSize); EVP_MD_CTX_free(evp_ctx);
...
//     // plaintext
//     std::string plaintext("This is a longer secret message that needs to be
//     encrypted.");
...
//     EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, (unsigned
//     char*)key.data(), (unsigned char*)iv.data());
...
//     // Decrypt init
//     EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, (unsigned
//     char*)key.data(), (unsigned char*)iv.data()); int plaintext_len = 0;
//     EVP_DecryptUpdate(ctx, (unsigned char*)decrypted_sw.data(),
//     &plaintext_len, (unsigned char*)ciphertext.data(), ciphertext.size());
//     final_len = 0;
//     EVP_DecryptFinal_ex(ctx, (unsigned char*)decrypted_sw.data() +
//     plaintext_len, &final_len); EVP_CIPHER_CTX_free(ctx); plaintext_len +=
//     final_len; decrypted_sw.resize(plaintext_len);
...
//     // encrypt with engine
//     ctx = EVP_CIPHER_CTX_new();
//     EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), engine, (unsigned
//     char*)key.data(), (unsigned char*)iv.data());
...
//     ctx = EVP_CIPHER_CTX_new();
//     EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), engine, (unsigned
//     char*)key.data(), (unsigned char*)iv.data()); int plaintext_len_engine =
//     0; EVP_DecryptUpdate(ctx, (unsigned char*)decrypted_engine.data(),
//     &plaintext_len_engine, (unsigned char*)engine_ciphertext.data(),
//     engine_ciphertext.size()); int final_len_engine = 0;
//     EVP_DecryptFinal_ex(ctx, (unsigned char*)decrypted_engine.data() +
//     plaintext_len_engine, &final_len_engine); EVP_CIPHER_CTX_free(ctx);
...
//     // Encryption with Openssl
//     EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
//     EVP_EncryptInit_ex(ctx, EVP_chacha20(), nullptr, (unsigned
//     char*)key.data(), (unsigned char*)nonce.data());
...
//     decrypted_sw.resize(ciphertext_len);
//     ctx = EVP_CIPHER_CTX_new();
//     EVP_DecryptInit_ex(ctx, EVP_chacha20(), nullptr, (unsigned
//     char*)key.data(), (unsigned char*)nonce.data()); int plaintext_len = 0;
//     EVP_DecryptUpdate(ctx, (unsigned char*)decrypted_sw.data(),
//     &plaintext_len, (unsigned char*)ciphertext.data(), ciphertext.size());
//     final_len = 0;
//     EVP_DecryptFinal_ex(ctx, (unsigned char*)decrypted_sw.data() +
...
//     // encrypt with engine
//     ctx = EVP_CIPHER_CTX_new();
//     EVP_EncryptInit_ex(ctx, EVP_chacha20(), engine, (unsigned
//     char*)key.data(), (unsigned char*)nonce.data());
...
//     decrypted_engine.resize(ciphertext_len);
//     ctx = EVP_CIPHER_CTX_new();
//     EVP_DecryptInit_ex(ctx, EVP_chacha20(), engine, (unsigned
//     char*)key.data(), (unsigned char*)nonce.data()); int plaintext_len_engine
//     = 0; EVP_DecryptUpdate(ctx, (unsigned char*)decrypted_engine.data(),
//     &plaintext_len_engine, (unsigned char*)engine_ciphertext.data(),
//     engine_ciphertext.size()); int final_len_engine = 0;
//     EVP_DecryptFinal_ex(ctx, (unsigned char*)decrypted_engine.data() +
...
//     memcmp(alice_shared_secret, bob_shared_secret, alice_shared_secret_len)
//     != 0 ) {
//         // error handling
//         FAIL();
//     }
...
//     EXPECT_EQ(memcmp(shared_secret1, shared_secret2, shared_secret_len), 0);
...
//   std::string path_to_bob_key = "/home/glaum/engine/keys/bob_pkey.pem";
//   std::string path_to_alice_pubkey =
//   "/home/glaum/engine/keys/alice_pubkey.pem"; std::string path_to_bob_pubkey
//   = "/home/glaum/engine/keys/bob_pubkey.pem";

//   // load alice key
//   EVP_PKEY *alice_pkey_engine = nullptr;
//   alice_pkey_engine = ENGINE_load_private_key(engine,
//   path_to_alice_key.c_str(),
//                                               nullptr, nullptr);
...
//   bob_pubkey_engine = ENGINE_load_public_key(engine,
//   path_to_bob_pubkey.c_str(),
//                                              nullptr, nullptr);
...
//   ret = EVP_PKEY_derive_init(ctx1);
//   EXPECT_EQ(ret, 1);
//   std::cout << "\nCalling derive set peer\n";
//   printf("Memloc of bobs key: %p\n", bob_pubkey_engine);
