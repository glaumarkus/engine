...
/* sha256 mapping */
size_t sha256_size() { return sizeof(Factory::FactoryDigest *); }
...
int sha256_init(struct engine_factory_instance *instance, EVP_MD_CTX *ctx) {
  ...
  if (factory != nullptr) {
    ...
    auto *digest =
        static_cast<Factory::FactoryDigest *>(EVP_MD_CTX_md_data(ctx));
    auto *dest = std::memmove(digest, factory_digest.get(),
                              sizeof(Factory::FactoryDigest));
    if (dest != nullptr) {
      ok = digest->Init(ctx);
    }
  }
...
int sha256_update(EVP_MD_CTX *ctx, const void *in, size_t len) {
  auto *digest =
      reinterpret_cast<Factory::FactoryDigest *>(EVP_MD_CTX_md_data(ctx));
