...
int chacha20_init(engine_factory_instance* instance, EVP_CIPHER_CTX *ctx, const unsigned char *key,
                  const unsigned char *iv, int enc) {
  int ok = 0;
  auto *factory = static_cast<Factory::SoftwareImpl::EngineFactory*>(instance->instance);
  if (factory != nullptr)
...
int aes256_gcm_init(engine_factory_instance* instance, EVP_CIPHER_CTX *ctx, const unsigned char *key,
                    const unsigned char *iv, int enc) {
  int ok = 0;
  auto *factory = static_cast<Factory::SoftwareImpl::EngineFactory*>(instance->instance);
  if (factory != nullptr)
